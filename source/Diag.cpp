...
{
	const std::string override_ = environmentVariable( "CARTRIDGE_LOG_DIR" );
	if( !override_.empty() )
		return override_;
...
void write( const char* level, const std::string& message )
{
	...
	if( g_console )
		std::fprintf( stderr, "%s %s: %s\n", level, kAppName, message.c_str() );

	if( !g_ready )
		return;

	// Opened and closed per line rather than held open, as in orrery: a plugin
	// holding a file handle for the life of the host is a worse trade than the
	// open() cost, and nothing is left buffered when the host exits.
...
void init()
{
	...
	info( std::string( "cartridge loaded build=" ) + __DATE__ + " " + __TIME__ );
}
